...
### Added Authentication Scopes

Added comprehensive feature for OAuth 2.0 authentication scopes used in the `request_server_side_access()` method. The documentation includes details about the three supported scopes:

- **`AuthScope.OPEN_ID`** - For unique user identification on the server
- **`AuthScope.EMAIL`** - For accessing the user's email address
- **`AuthScope.PROFILE`** - For accessing basic profile information

This documentation helps developers understand what permissions their game server will receive when requesting server-side access.
