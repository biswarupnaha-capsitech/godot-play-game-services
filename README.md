...
The plugin uses OAuth 2.0 authentication scopes to request specific permissions from the user during the authentication process. These scopes are used in the `request_server_side_access()` method to define what user information your game server will be allowed to access.

#### Supported Auth Scopes

The plugin supports the following authentication scopes:

- **`AuthScope.OPEN_ID`** - Allows the game server to identify the user uniquely. This scope provides access to the user's unique identifier (subject claim), which is essential for securely identifying the player on the server side.

- **`AuthScope.EMAIL`** - Grants access to the user's email address. This scope is useful if your game server needs to send notifications or correspondence to the player's registered email account.

- **`AuthScope.PROFILE`** - Provides access to the user's basic profile information such as name, picture, and other public profile details. This is useful for displaying personalized information in your game or on your backend.

#### Usage Example

These scopes are automatically requested when you call the `request_server_side_access()` method in the Sign-In client:

```gdscript
# Request server-side access with all available scopes
play_games_sign_in_client.request_server_side_access(
    server_client_id,
    force_refresh_token
)
```

The scopes ensure that your game server receives an authorization code that can be exchanged for an access token with the necessary permissions to access user information as defined by the requested scopes.
